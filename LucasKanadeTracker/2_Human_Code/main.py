...
if __name__ == '__main__':
    result_dir = '../data_enpm673/results/human/'
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    cars_data = glob.glob('../data_enpm673/human/*.jpg')
    cars_data.sort()
    frame = cv2.imread(cars_data[0])
    frame = cv2.resize(frame, (320, 240))
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    rect_list = []
    rect = np.array([170, 110, 185, 130])# Hard Coded
    ...
    for i in range(1, len(cars_data)):
        ori_rects = np.load("humanseqrects.npy")
        ...
        if i in range(len(cars_data)):
            cv2.imwrite(os.path.join(result_dir, 'human_frame_corrected_{}.jpg'.format(i)), copy_temp_img)
    ...
    np.save('humanseqrects-wcrt.npy', rect_list)
